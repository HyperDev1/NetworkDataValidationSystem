...
class MetaFetcher(NetworkDataFetcher):
    ...
    # Daily aggregations require ~3 days to stabilize (Meta API documented behavior)
    # We use T-3 to ensure complete and accurate data
    DATA_DELAY_DAYS = 3
    ...
    def fetch_data(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        Note: Meta has a reporting delay of ~3 days. The caller (validation_service)
        is responsible for passing the correct shifted date range (T-3).
        ...
        """
        print(f"      [INFO] Fetching Meta Audience Network data (T-3 daily mode)...")
        ...
        # Query parameters - use simple date format (YYYY-MM-DD) per Meta API docs
        # and aggregation_period: day for stable daily data
        query_params = {
            ...
            "aggregation_period": "day",  # Use daily aggregation for stable data
        }
